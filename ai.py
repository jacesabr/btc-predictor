...
PREMORTEM_GATE (fill BEFORE writing POSITION):
  ...
  P2 (P0 cites a low-volume extreme-flow noise-trap. STRICT TEST — ALL THREE must be true: (1) taker_total < 2 BTC; (2) BSR > 5.0 STRICTLY OR BSR < 0.2 STRICTLY (4.667 is NOT > 5.0; 2.5 is NOT > 5.0; only values like 5.1, 6, 8 qualify); (3) spot whale flow does NOT corroborate POSITION (UNAVAILABLE/NO_DATA counts as not-corroborating). Show the arithmetic before answering: taker_total=<value>; <value> < 2? YES/NO. BSR=<value>; <value> > 5.0? YES/NO; <value> < 0.2? YES/NO. P2=YES only if condition-1=YES AND (BSR>5.0=YES OR BSR<0.2=YES) AND condition-3=YES. If any condition fails, P2=NO.)
...
SPECIALIST_AUTHORITY_GATE (fill BEFORE POSITION; verdict can OVERRIDE a NEUTRAL draft):
  ...
  SAG6 microstructure-corroborates-ensemble (HARD PRECONDITION FOR SAG5) — the trailing-window ensemble can be wrong about the CURRENT bar; require live current-bar microstructure to corroborate the ensemble before letting it override a NEUTRAL draft. Re-list the 5 C1 microstructure signals (order_book_imbalance sign | taker BSR (>1=UP, <1=DOWN) | A/D slope sign | last 4-bar net price change sign | MACD histogram sign). STRICT LABELING RULE: FLAT applies ONLY when the value is EXACTLY 0 (e.g., A/D slope=+0, BSR=1.000, last 4-bar net change=0.0). Any non-zero negative value is DOWN; any non-zero positive value is UP — even if small (e.g., OBI=-4.14% is DOWN, not FLAT; OBI=+3.16% is UP, not FLAT). Then mark MATCH / OPPOSE / FLAT vs the SAG1 ensemble direction (FLAT does NOT count as MATCH). Count strict MATCH. SAG6=YES if MATCH >= 3 of 5. Else SAG6=NO. Cite all 5 raw values, the UP/DOWN/FLAT label per the strict rule, the MATCH/OPPOSE/FLAT vs ensemble, and the final count.
  SAG5 verdict — IF SAG1=YES AND SAG2=YES AND SAG3=INSUFFICIENT AND SAG4=NONE AND SAG6=YES → POSITION MUST be ABOVE (ensemble UP) or BELOW (ensemble DOWN); ARGUMENT MUST lead with the ensemble's weighted vote; SURVIVES_STEELMAN MUST be YES. Any "(#NNN POSTMORTEM)" or "X% rate" claim NOT literally in the HISTORICAL SIMILARITY ANALYST block is a hallucination — do not cite. IF SAG6=NO → GATE_INACTIVE — do NOT force-flip a NEUTRAL draft; the current-bar microstructure does not corroborate the trailing-window ensemble, so the NEUTRAL draft stands. Else GATE_INACTIVE.
...
async def run_binance_expert(
    ...
    try:
        ...
        elapsed = time.time() - t0
        logger.warning(
            "Binance expert TIMEOUT after %.1fs (inner _api_call timeout=42s) — "
